define(function () {
  ...
  // -------------------------------------------
  var assembleCustomManager = function (item, one) {
    item.customManager = {};
    item.customManager.active = false;
    item.customManager.name = '王思聪';
    item.customManager.from = '沃锐猎头';
  };
  ...
  return function translatePosted(res) {
    ...
    res.position.forEach(function (one) {
      ...
      assembleCustomManager(item, one);
